...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    try:
        ...
        # Setup frontend panel directly
        from .frontend import async_setup_entry as async_setup_frontend
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    try:
        # Unload frontend
        from .frontend import async_unload_entry as async_unload_frontend

        unload_ok = await async_unload_frontend(hass, entry)
        if not unload_ok:
            return False
                    ...
                    results.append(
                        {
                            ...
                            "has_maintained_label": LABEL_MAINTAINED
                            in (entity.labels or set()),
        ...
        try:
            ...
            try:
                entity_reg.async_update_entity(
                    entity_id, new_entity_id=new_id, name=friendly_name
                )

                # Add maintained label
                labels = set(entity.labels or set())
                labels.add(LABEL_MAINTAINED)
                entity_reg.async_update_entity(new_id, labels=labels)

                _LOGGER.info("Successfully renamed entity %s to %s", entity_id, new_id)
                ...
                raise HomeAssistantError(f"Failed to rename entity: {err}")
            ...
            for entity in entities_to_rename:
                try:
                    ...
                    if not dry_run:
                        # Actually rename
                        entity_reg.async_update_entity(
                            entity.entity_id, new_entity_id=new_id, name=friendly_name
                        )
    ...
    async def websocket_get_areas(hass, connection, msg):
        ...
        for area in area_reg.areas.values():
            entity_count = sum(
                ...
                or (
                    entity.device_id
                    and dr.async_get(hass).async_get(entity.device_id)
                    and dr.async_get(hass).async_get(entity.device_id).area_id
                    == area.id
                )
            )
            areas.append(
                {"area_id": area.id, "name": area.name, "entity_count": entity_count}
            )
    ...
    async def websocket_get_entities_by_area(hass, connection, msg):
        ...
        if not area:
            connection.send_error(
                msg["id"], "area_not_found", f"Area {area_name} not found"
            )
            return
    ...
    async def websocket_rename_entities(hass, connection, msg):
        ...
        for entity_id in entity_ids:
            try:
                ...
                results.append(
                    {"success": False, "entity_id": entity_id, "error": str(err)}
                )
