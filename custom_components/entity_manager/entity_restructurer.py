...
class EntityRestructurer:
    ...
    async def load_structure(self, ws_client: Optional[Any] = None) -> None:
        ...
        try:
            ...
            if response.get("success"):
                ...
                logger.info(
                    f"Successfully loaded {len(self.areas)} areas via WebSocket"
                )
            else:
        ...
        try:
            ...
            if response.get("success"):
                ...
                logger.info(
                    f"Successfully loaded {len(self.devices)} devices via WebSocket"
                )
            else:
        ...
        try:
            ...
            if response.get("success"):
                ...
                logger.info(
                    f"Successfully loaded {len(self.entities)} entities from registry"
                )
    ...
    def generate_new_entity_id(
        self, entity_id: str, state_info: Dict[str, Any]
    ) -> Tuple[str, str]:
        """
        ...
        try:
            ...
            if device:
                ...
                if device_override:
                    ...
                    device_friendly_name = device.get("name_by_user") or device.get(
                        "name"
                    )
            ...
            logger.debug(
                f"Generated new entity ID: {entity_id} -> {new_entity_id} ({friendly_name})"
            )
    ...
    async def analyze_entities(
        ...
        try:
            ...
            for state in states:
                ...
                new_entity_id, friendly_name = self.generate_new_entity_id(
                    entity_id, state
                )
