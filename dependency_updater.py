...
class DependencyUpdater:
    ...
    async def update_scene_entities(self, scene_id: str, scene_numeric_id: str, 
                                  old_entity_id: str, new_entity_id: str) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=self.headers) as response:
                if response.status == 200:
                    ...
                    logger.error(f"Fehler beim Abrufen der Automation {automation_numeric_id}: {response.status}, Response: {text}")
                    return None
    ...
    async def update_automation_entities(self, automation_id: str, automation_numeric_id: str,
                                       old_entity_id: str, new_entity_id: str) -> bool:
                ...
                if config:
                    ...
                    if old_entity_id in config_str:
                        ...
                        success = await self.update_automation_entities(
                            automation_entity_id, automation_numeric_id, old_entity_id, new_entity_id
                        )
                        if success:
