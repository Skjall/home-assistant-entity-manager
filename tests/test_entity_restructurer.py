...
class TestEntityRestructurer:
    ...
    def test_normalize_name(self, restructurer):
        ...
        assert (
            restructurer.normalize_name("Test  Multiple  Spaces")
    ...
    def test_get_entity_type(self, restructurer):
        ...
        assert (
            restructurer.get_entity_type("sensor.test", "temperature") == "temperatur"
        )
        assert (
            restructurer.get_entity_type("sensor.test", "humidity")
            == "luftfeuchtigkeit"
        )
        ...
        # Test binary sensor
        assert (
            restructurer.get_entity_type("binary_sensor.test", "motion") == "bewegung"
        )
    ...
    def test_generate_new_entity_id_with_area_override(self, restructurer):
        ...
        restructurer.naming_overrides._overrides = {
            "areas": {"area1": {"name": "Büro"}}
        }

        restructurer.areas = {"area1": {"area_id": "area1", "name": "Office"}}
        restructurer.devices = {
            "device1": {
                "id": "device1",
                "name": "Desktop Light",
    ...
    def test_generate_new_entity_id_no_device(self, restructurer):
        ...
        restructurer.areas = {"area1": {"area_id": "area1", "name": "Kitchen"}}
        restructurer.entities = {
            "light.kitchen_light": {
    ...
    async def test_load_structure_with_websocket(self, restructurer):
        ...
        mock_ws._receive_message = MagicMock(
            side_effect=[
                {
                    "id": 1,
                    "success": True,
